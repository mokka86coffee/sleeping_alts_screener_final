...
def _objections(star: dict) -> list[dict]:
    ...
    # 5. Транш в горизонте — ПО ЕГО РАЗМЕРУ, а не по одному сроку.
    #
    #    Возражение выносилось на ЛЮБОЙ транш ближе семи дней, хотя
    #    ступени в этом же модуле говорят прямо: доля ниже пяти
    #    процентов обращения — фон, «держать», такой объём
    #    переваривается дневным оборотом и до цены почти не доходит.
    #    Из-за этого ENA с траншем в 2.71% получала заголовком
    #    «входить перед раздачей значит покупать у того, кто выходит»
    #    — приговор от величины, которая ничего не решает (разбор
    #    31.08). Транш при этом никуда не девается: он остаётся
    #    фактом в строке разлока и пилюлей, просто перестаёт быть
    #    вердиктом.
    #
    #    Горизонт тоже берём из ступени, а не общей константой —
    #    ровно то, о чём докстрока модуля: перед крупным выходят за
    #    неделю-две, мелкому хватает дня события. Доля неизвестна —
    #    говорим ИМЕННО ЭТО, а не про раздачу: приговор по величине,
    #    которой нет, здесь запрещён так же, как в unlock_tier.
    ud = star.get("unlockDays")
    try:
        ud = int(ud) if ud is not None else None
    except (TypeError, ValueError):
        ud = None
    _tier = unlock_tier(star.get("unlockPctFloat"))
    if ud is not None and 0 <= ud <= _tier["days"]:
        when = "сегодня" if ud == 0 else f"через {ud} дн"
        if _tier["share"] is None:
            out.append({
                "short": f"транш {when}, доля неизвестна",
                "full": (f"разлок {when}, доля от обращения не посчитана — "
                         f"размер предложения неизвестен, а решает именно "
                         f"он; посмотреть календарём глазами"),
                "lift": "снимется, когда доля транша станет известна"})
        elif _tier["act"] != "держать":
            out.append({
                "short": f"транш {_tier['share']:.0f}% {when}",
                "full": (f"разлок {_tier['share']:.0f}% обращения {when} — "
                         f"предложение приходит в стакан и встречает любой "
                         f"ход сверху; входить перед раздачей значит "
                         f"покупать у того, кто выходит"),
