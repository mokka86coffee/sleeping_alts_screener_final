...
KEEP_SERIES = ("spark_1d", "spark_vol", "oi_spark", "liq_fresh")
# oi_spark и liq_fresh добавлены 29.08 после раскопки: strip_series
# молча резал ВСЕ списки, кроме двух спарков, — и хвост OI с зонами
# ликвидаций умирали по дороге к звёздам, хотя считались честно.
# Старое поле liqFresh в звёздах было мёртвым с рождения по той же
# причине. Вес добавки копеечный: 24 числа и до восьми компактных
# словарей на монету. Крикун в render_page теперь ловит такую
# пропажу у всех разом — немых ножей в трубе больше нет.
SPARK_POINTS = 24
...
def collect_metrics(symbol: str, quote_volume_24h: float = 0.0) -> dict:
    ...
    return {
        ...
        "liq_fresh": liq_fresh,
        # Хвост часового OI для мини-ряда «формы суток» (29.08):
        # число прячет форму — рост плеча на пампе и удержание после
        # читаются только рядом. Полный ряд в JSON не уходит.
        "oi_spark": [round(v, 1) for v in (oi_hourly or [])[-24:]],
