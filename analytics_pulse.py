...
def snapshot(c: Candidate) -> dict:
    ...
    if hour is not None:
        ...
        # ЧЕТЫРЕ СЕССИИ, НЕ ТРИ (12.09, владелец: «у нас 3 сессии, по факту их 4»). Сидней —
        # отдельный рынок, и на LSK 13.09 вся вертикаль прошла именно в нём, до открытия Токио.
        # Часы — как в LuxAlgo Sessions на графике владельца, UTC:
        #   Сидней   21:00–06:00   Токио 00:00–09:00   Лондон 07:00–16:00   Нью-Йорк 13:00–22:00
        # Окна перекрываются намеренно: ликвидность перетекает, а не переключается по звонку.
        win = []
        ...
        # СТЫК СЕССИЙ (12.09, владелец: «крупному нужно понять, будут ли новые покупки в этой
        # сессии — это разные материки, страны и люди»). На стыке крупный делает пробный ход и
        # смотрит, отвечает ли новая сессия; поэтому пузыри у LSK стояли именно в переходах:
        # покупки на стыке Нью-Йорк→Сидней, продажи на стыке Сидней→Токио. Помечаем открытие
        # каждой сессии и первый час после него — читать пробу и ответ.
        opens = {21: "sydney", 0: "tokyo", 7: "london", 13: "us"}
        out["sess_open"] = opens.get(hour, "")
        out["sess_edge"] = bool(out["sess_open"])
    ...
    # Г-16: Клингер тем же 4h-масштабом. Пишем СЕЙЧАС, чтобы копилась
    # история под проверку ложных срабатываний (ретро мерило только
    # ходы). Магнитуда KVO — деньги, между монетами несравнима,
    # поэтому в точке лишь знак к сигнальной и крест на последнем
    # баре: kv ±1, kvx ±1 (нет креста — поля нет).
    kv = raw.get("klinger_4h") or {}
    if kv:
        out["kv"] = 1 if kv.get("above") else -1
        if kv.get("crossUp"):
            out["kvx"] = 1
        elif kv.get("crossDn"):
            out["kvx"] = -1
...
def vortex_state(symbol: str, back: int = 48) -> dict:
    ...
    ход вниз в 10 из 14, медиана −6%, НО четыре случая вверх — это
    +18, +34, +52: ONG 22.08, BLESS 04.08, самые большие ходы выборки.
    Значит сошедшийся вихрь говорит не «вниз», а «импульс кончился,
    дальше выброс в любую сторону». Образец — ZEC 03.06: линии в
    единице, потом −66% за день, потом ×2.5 за три месяца. Куда пойдёт
    — решает фундамент за монетой, не индикатор.
    Поэтому это СТОРОЖ волатильности, не направления: предупреждение в
    тексте сюжета, а не смена шаблона.
    """
    pts = (_load() or {}).get(symbol) or []
    pts = [r for r in pts if r.get("vi_p") is not None and r.get("vi_m") is not None]
    if len(pts) < 8:
        return {}
    win = pts[-back:]
    sp = [float(r["vi_p"]) - float(r["vi_m"]) for r in win]
    cur = sp[-1]
    hi, lo = max(sp), min(sp)
    ...
    if abs(cur) <= 0.05:
        ...
        out["state"] = "shrinking"           # импульс вверх выдыхается
    elif lo <= -0.35 and cur < 0 and cur >= lo * 2 / 3:
        out["state"] = "shrinking"           # импульс вниз выдыхается
    return out
...
def for_symbol(symbol: str) -> dict:
    ...
    for name, ref in (("prev", pts[-2]), ("h6", _pick(pts, 6)),
        ...
        for key in DELTA_KEYS:
            ...
            # Процент по плечу — симметрично цене (01.09). Абсолютная
            # дельта открытого интереса ни с чем не сравнима между
            # монетами, а доля сравнима: минус десять процентов за час
            # значит одно и то же и на пяти миллионах, и на восьмидесяти.
            if key == "oi_usd" and b:
                d["oi_pct"] = round((a / b - 1) * 100, 2)
        if d:
