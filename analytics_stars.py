...
def _star_card(c: Candidate | None) -> dict:
    ...
    # Доля спота. Ключ к вопросу «откуда деньги» (разбор 26.08): у BTR
    # спота не было ВОВСЕ, цену задавал только контракт, и разгон
    # оплатили накопленным плечом. Ноль здесь — не «не измерено», а
    # осмысленный ответ «спотового рынка нет», поэтому ключ ставим и
    # при нуле; отсутствие raw-поля — другое дело, тогда ключа нет.
    sr = (getattr(c, "raw", None) or {}).get("spot_ratio")
    if isinstance(sr, (int, float)):
        out["spotShare"] = round(float(sr), 4)
