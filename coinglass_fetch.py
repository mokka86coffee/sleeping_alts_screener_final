...
ЧТО СНИМАЕТ И ПОЧЕМУ ИМЕННО ТАК (по фактам пробника 29.08, MAGMA):
  ...
  · ФЬЮЧЕРСНАЯ ДЕЛЬТА /futures/aggregated-cvd/history — в ОДНОЙ точке
    ...
    запрос на монету. Окно — сутки ЗАКРЫТЫХ баров, с 04.09 по 30 минут (карточка просит ряд
...
# 04.09: шаг полчаса вместо часа — минимальный на Startup (30m — 90 дней,
# 1h — 180, ниже только Standard). Прогон идёт раз в полчаса, и каждый
# прогон получает НОВЫЙ закрытый бар, а не тот же часовой. Окно — те же
# сутки, только 48 баров. Запросов столько же (один на монету и ряд),
# лимит 80/мин не трогаем.
INTERVAL = "30m"
WINDOW = 48                         # закрытых баров: сутки — просит карточка (48 × 30m)
MAX_COINS = 250                     # не потолок, а страховка от сбоя журнала (03.09, правило владельца: лишние монеты убираются руками, срез журнал не режет)
