...
Основное правило при доработке семейства - не изменять действующий функционал общих модулей (конфигов,
аналитики, индикаторров и т.д), в общие модули можно только добавлять. Другие детекторы и их функционал
здесь не используются — это правило семейства.
...
def build_drop_context(bars: list[Bar]) -> DropContext:
    ...
    # Рост до пика считается ВСЕГДА и раньше всего остального.
    # Ему не нужны ни дно, ни падение: величина меряется от базы до
    # вершины, и что случилось после вершины — отдельный вопрос.
    #
    # Прежде блок стоял ниже проверки len(after), и монета, чей пик
    # пришёлся на последние дни, до него не доходила: after короче
    # минимума, ранний выход, growth_x = 0. То есть вето на
    # экстремальный рост не применялось именно к тем, кто стоит на
    # историческом максимуме, — к AKE и ON с ценой, совпадающей с
    # верхней зоной. Между тем толпа с прибылью там никуда не
    # делась, а уровни под ней ещё не проверены сливом.
