...
def _count_rallies(after: Sequence[Bar]) -> tuple[int, float]:
    ...
    """
    # Три величины во ВСЕХ точках возврата, включая ранние.
    # Кортеж разной длины из одной функции — это ошибка, которая
    # прячется до первой монеты с короткой историей.
    if len(after) < 4:
        return 0, 0.0, 0.0
