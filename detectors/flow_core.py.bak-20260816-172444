...
class Bar:
    ...
    def buy_quote(self) -> float:
        """Тейкерные покупки в котируемой валюте.

        Имя, под которым сторону читает taker. Синоним buy_volume:
        объёмы в ядре и так приходят в quote.
        """
        return self.buy_volume

    @property
    def buy_share(self) -> float:
        """Доля тейкерных покупок в обороте бара, 0..1.

        Считается по сторонам, а не по знаку закрытия. Нет сторон
        (агрегат без них) — нейтральные 0.5, а не ноль: ноль
        означал бы «продавали всё», и это неправда.
        """
        total = self.buy_volume + self.sell_volume
        if total <= 0:
            return 0.5
        return _clip(self.buy_volume / total, 0.0, 1.0)
...
class DropContext:
    ...
    # Сколько из них ВЕРНУЛИСЬ на дно, не пробив его глубже допуска.
    #
    # Отдельно от rallies, потому что отвечает на другой вопрос.
    # Отскок с возвратом на прежнее дно означает, что предложение по
    # этой цене снова разобрали: чем больше таких, тем живее монета.
    # Отскок, после которого цена ушла ниже, — раздача, и в этот счёт
    # он не входит, хотя отработанным разгоном остаётся.
    held_rallies: int = 0
...
def build_drop(bars: Sequence[Bar],
    ...
    return DropContext(
        ...
        rallies=rs.count,
        held_rallies=rs.held_count,
        max_rally_pct=rs.max_pct,
        hold_pct=rs.hold_pct,
...
@dataclass
class RallyStats:
    """Итог обхода разгонов. Структура, а не кортеж, и это по делу.

    Прежняя редакция возвращала кортеж, и однажды из трёх точек
    выхода вернулись два значения вместо трёх — распаковка на
    стороне вызова падала только на монетах с короткой историей,
    то есть не сразу. Добавление поля в структуру такого класса
    ошибок не создаёт вовсе.
    """
    count: int = 0
    held_count: int = 0
    max_pct: float = 0.0
...
def _count_rallies(after: Sequence[Bar]) -> RallyStats:
    ...
    # По паре на каждый засчитанный разгон: опора до него и самый
    # низкий минимум после него.
    #
    # Второе число живое: пока идёт поиск следующего разгона, опора
    # едет вниз за ценой, и вместе с ней едет запись. Иначе разгон,
    # вернувшийся ровно на дно, а через неделю провалившийся, до
    # конца обхода считался бы удержавшим.
    ...
    for b in after:
        ...
        if not in_rally:
            if b.low < base:
                base = b.low
                # Опора уехала вниз — уезжает и минимум последнего
                # разгона: удержание меряется по факту, а не по
                # точке, где разгон закончился.
                if spans:
                    spans[-1][1] = base
    ...
    # Удержавшие считаются по тому же допуску, что и свойство held:
    # порог один на понятие, иначе «дно удержано» в двух местах
    # означало бы разное.
    held_count = sum(
        1 for lo, hi in spans
        if lo > 0 and (hi - lo) / lo * 100.0 >= -HOLD_TOL_PCT
    )

    return RallyStats(count=count, held_count=held_count,
                      max_pct=best, hold_pct=hold)
