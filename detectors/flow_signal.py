...
SubcaseSignal либо None. Ничего не считает сам сверх своей фигуры,
не лезет в сеть, не знает о других подкейсах.
...
class SubcaseSignal:
    ...
    Внутренний тип семейства. Наружу через detectors/__init__ выходит
    только FlowSignal из flow.py — сведённый результат диспетчера.
    Разные сущности, разные имена: подкейс описывает одну фигуру,
    FlowSignal описывает монету.

    score — сырой вклад в шкале 0..100 ДО сведения. Приведением к
    диапазону семейства 45..100 занимается flow.py.
    """
    ...
    @property
    def cut(self) -> float:
        """Во сколько раз множители урезали базовый скор.

        Меньше 1 — фигуру ослабили, больше 1 — усилили. Читается
        при калибровке: если сильные фигуры систематически падают
        ниже порога, дело в множителях, а не в базовой оценке.
        """
        if self.base_score <= 0:
            return 1.0
...
class Subcase(Protocol):
    ...
    нужен не ради типизации как таковой, а чтобы flow.py мог держать
    реестр подкейсов списком и не знать их внутренностей.
    """
...
#
# Возвращается ПРИЧИНА отказа, а не булево: при разборе прогона
# нужно знать, почему монета выпала, иначе калибровать нечего.
# Подкейсы вправе игнорировать текст и проверять на истинность.

def veto_common(ctx: FlowContext, require_zones: bool = True) -> str | None:
    """Возвращает причину отказа либо None.

    Здесь остаются ТОЛЬКО отсечки «нечего считать»: нет истории, нет
    материала для фигуры. Суждения о качестве кейса из ядра убраны —
    фильтром служит сама стратегия.

    Вето по экстремальному росту снято. Оно отвечало на вопрос «во
    сколько раз выросла перед падением», а трактовалось как «сколько
    народу сидит в убытке», и подмена не лечится подбором окна. Цена
    ошибки видна на BLESS: восьмикратка внутри окна снимала монету,
    которая в этот момент шла в рост после исчерпания продавца.
    Навес сверху теперь читается формой пиков вортекса — прямо, а не
    через отношение двух цен.
    """
    if not ctx.ready:
        return "мало истории"

    if require_zones and not ctx.zones:
        return "нет живых зон"

    return None


def veto_bullish(ctx: FlowContext, require_zones: bool = True) -> str | None:
    """Дополнительные вето для подкейсов, играющих от разворота.

    Отделены от veto_common: flow_fuel описывает расположение
    предложения, а не разворот, и обвал дельты для него не помеха —
    при сливе предложение расположено там же, где при накоплении.
    """
    base = veto_common(ctx, require_zones=require_zones)
    if base:
        return base
