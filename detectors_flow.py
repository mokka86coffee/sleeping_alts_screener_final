...
import math
# Медиана из stdlib, а не из analytics_indicators: детекторы не
# зависят от analytics, и заводить зависимость ради одной функции,
# которая есть в стандартной библиотеке, не стоит.
...
from detectors_flow_config import cycle_done
...
import detectors_flow_dormant as flow_dormant
...
def _cycle_up_x(ctx: FlowContext) -> float:
    """Во сколько раз цена ушла от дна цикла.

    Дно берётся из DropContext (окно BOTTOM_LOOKBACK_DAYS), а не из
    метрик: там up_from_low считается по окну в 60 дней, и у монеты,
    которая уже поехала, локальное дно уползает вверх следом за
    ценой — кратность выходит заниженной ровно у тех, кого правило
    должно ловить.

    Ноль означает «дна не нашли»: величины нет, и потребитель обязан
    отличать это от единицы. Порог, сработавший на нуле, выбросил бы
    монету, про которую мы ничего не знаем.
    """
    bottom = float(ctx.drop.bottom_price or 0.0)
    if bottom <= 0 or ctx.price <= 0:
        return 0.0
    return ctx.price / bottom


def _life_vwap(symbol: str) -> dict:
    """VWAP всей жизни контракта и дистанция цены до него (И-3).

    Средний безубыток всего капитала, прошедшего через монету: на
    разборе BLESS апрельский вынос умер ровно на этой линии, и
    крупнейшая агрессивная продажа истории легла на касание. Считается
    по тем же недельным свечам, что _life_span, — ряд уже в RunCache,
    сети ноль.

    VWAP = Σ(quote) / Σ(base): quote-объём свечи и есть Σ(цена·штуки)
    внутри неё, поэтому отношение двух сумм даёт средневзвешенную цену
    без выбора, какой ценой представлять бар.

    Пустой словарь — «жизни не видели», как у остальных life-величин.
    VWAP волн (rally_vwap из И-3) сюда не входит: он требует вершин
    разгонов из _count_rallies, а менять её сигнатуру без потребителя
    не стоит — она недавно чинена после кортежей разной длины.
    """
    kl = klines_1w(symbol)
    if not kl or len(kl) < 2:
        return {}
    ...
    vwap = quote / base
    out = {"vwap": round(vwap, 10)}
    if last_close > 0 and vwap > 0:
        out["dist_pct"] = round((last_close / vwap - 1) * 100, 1)
    return out


def _funding_stats(symbol: str) -> dict:
    """Фандинг рядом, а не точкой (И-2).

    Точка ставки не значит почти ничего: на разборе BLESS текущее
    значение породило теорию перекоса, которую история из месяца
    опровергла за секунду. Значение имеют форма ряда: медианы окон,
    доля отрицательных ставок, стрик одного знака. drift — сдвиг
    короткой медианы против длинной — ранний признак конца баланса,
    ЕСЛИ цена при этом плоская; плоскость цены здесь не проверяется,
    она уже есть в пейлоаде, складывать величины — дело читателя.

    Единицы — проценты за интервал, интервалов три в сутки:
    med7 — медиана ~7 дней (21 ставка), med30 — всей доступной
    истории (лимит Binance 100 ставок ≈ 33 дня). streak — знаковый:
    +N подряд положительных, −N подряд отрицательных, 0 если
    последняя ставка нулевая. Пустой словарь — истории меньше
    двух суток.
    """
    ...
    for r in hist or []:
        try:
            rates.append(float(r.get("fundingRate", 0)) * 100)
        except (TypeError, ValueError, AttributeError):
            continue
    if len(rates) < 6:
        return {}

    med7 = median(rates[-21:])
    med30 = median(rates[-90:])
    tail = rates[-90:]
    neg_share = sum(1 for r in tail if r < 0) / len(tail)
...
def _context_dict(ctx: FlowContext, hz: dict) -> dict:
    ...
    return {
        ...
        "drop": {
            ...
            # Дно цикла и кратность от него. Наружу идут обе: цена
            # дна нужна как уровень (журнал считает от неё, отчёт
            # рисует), кратность — как готовая величина, чтобы
            # каждый потребитель не делил заново и не разошёлся в
            # третьем знаке. Мерка одна на проект: и журнал, и вето
            ...
            # Сколько отскоков вернулись на дно, не пробив его.
            # rallies отвечает «сколько раз пробовали», held_rallies —
            # «сколько раз дно выдержало»; вторая величина и есть
            # живость монеты, и до сих пор её не существовало.
            "held_rallies": ctx.drop.held_rallies,
...
def detect_flow(
    ...
    # Вето по завершённому циклу. Монета, ушедшая от дна в
    # CYCLE_COMPLETE_X раз, свою волну отработала — семейство ищет
    # начало движения, а не его конец, и тратить на неё подкейсы
    # незачем.
    #
    # Проверка стоит ДО подкейсов и после контекста: раньше нет дна,
    # позже уже посчитано лишнее. Ноль кратности означает «дна не
    ...
    # И-1…И-3: ряды OI и фандинга и VWAP жизни — в пейлоад, к пробе.
    # Гейтов и множителей нет намеренно: порог выбирается по разбросу
    # рынка, а не придумывается. VWAP жизни вне сетевого гейта по той
    # же причине, что _life_span: недельки уже загружены метриками и
    # лежат в RunCache. Ряды OI и фандинга — сеть (по весу 1, с
    # кэшем), поэтому под allow_network; при выключенной сети ключей
    # просто нет — отсутствие честнее пустых нулей.
    ctx_dict["life_vwap"] = _life_vwap(symbol)
    if allow_network:
        ctx_dict["oi_hist"] = _oi_stats(symbol)
        ctx_dict["funding_hist"] = _funding_stats(symbol)
