...
def events(rows):
    ...
    for k, (t, nm) in enumerate(opens):
        ...
        # ЧИТАЕМ ДО КОНЦА СЕССИИ (19.09, владелец: «они подхватили просто не сразу»): приговор по первым трём барам
        # ставить нельзя — деньги приходят когда хотят. Пока сессия идёт и подхвата нет — «пока не подхватили» с числом
        # баров и остатком; пришли позже — «подхватили» с часом; «не подхватили» — только когда сессия закрылась.
        _bars_sess = [x for x in rows if t <= x["t"] < nxt]
        _open = (rows[-1]["t"] < nxt)                       # сессия ещё идёт
        _late = None
        if not ok and len(_bars_sess) > 3:
            for _k in range(4, len(_bars_sess) + 1):
                _seg = _bars_sess[:_k]
                _v = sum(qv(x) for x in _seg); _n = (norm or 0) * _k
                _vx = (_v / _n) if _n else 0
                _d = sum(dl(x) for x in _seg)
                _o0, _o1 = float(rows[i - 1].get("oi") or 0), float(_seg[-1].get("oi") or 0)
                _oc = (_o1 / _o0 - 1) * 100 if (_o0 and _o1) else 0
                _p = (float(_seg[-1]["px"]) / px0 - 1) * 100
                if _vx >= 1 and _p > 0 and (_oc > 0 or _d > 0):
                    _late = _k; ok = True; pxch = _p; delta = _d; oich = _oc; volx = _vx
                    newhi = max(float(x.get("h") or x["px"]) for x in _seg) > prev_hi
                    break
        ...
        if newhi and ok:
            ...
            kind = "не подхватил"
        if kind == "не подхватил" and _open:
            kind = "идёт"                                   # сессия не закрыта — приговора нет
        ...
        label = {"лестница": "лестница: подхват с максимумом", "пауза": "пауза: без максимума, дельта не тянет",
                 "не подхватил": "не подхватил", "идёт": "пока не подхватили — сессия идёт"}.get(kind) or ("раздача ×1 за сутки" if n_day == 1 else "раздача ×2 за сутки → хедж" if n_day == 2 else "раздача ×3+ за сутки")
        ...
        _left = max(0, int((nxt - rows[-1]["t"]).total_seconds() // 1800)) if _open else 0
        out.append(dict(t=t, i=i + 2, sess=nm, kind=kind, label=label, streak=n_day if kind == "раздача" else 0, pxch=pxch, delta=delta,
                        oich=oich, volx=volx, run=run, dnorm=(delta / norm if norm else 0),
                        bars=len(_bars_sess), left=_left, late=_late, open=_open))
    return out
...
WORD = {"лестница": "подхватили", "раздача": "раздача", "пауза": "ожидание", "не подхватил": "не подхватили", "идёт": "пока не подхватили"}
EXPLAIN = {
    ...
    "не подхватил": "сессия закрылась, а денег так и не пришло — пробу не приняли",
    "идёт": "сессия ещё идёт, денег пока не было; деньги приходят когда хотят, приговор — только по закрытии",
}
...
ACTION = {
    ...
    ("идёт", True): "ждать · сессия идёт, приговора нет",
    ("идёт", False): "ждать · сессия идёт, приговора нет",
}
