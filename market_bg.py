...
# закрытии Нью-Йорка в 22:00 UTC, а Нью-Йорк считались до 20:00 — журнал писал это как «сессии нет»):
# Токио 0–9, Лондон 7–16, Нью-Йорк 13–22. СИДНЕЙ ДОБАВЛЕН 12.09 (владелец: «у нас 3 сессии, по
# факту их 4»): 21–6 UTC, это отдельный рынок и отдельные деньги. На LSK 13.09 вся вертикаль
# с 0.24 до 0.36 прошла в сиднейскую сессию, до открытия Токио, — при трёх сессиях журнал писал
# бы это как азиатскую и потерял бы разницу между материками.
SESSIONS = (("Сидней", 21.0, 6.0), ("Токио", 0.0, 9.0), ("Лондон", 7.0, 16.0), ("Нью-Йорк", 13.0, 22.0))
...
def _session(now: datetime) -> dict:
    ...
    for name, a, b in SESSIONS:
        # СЕССИЯ ЧЕРЕЗ ПОЛНОЧЬ (12.09, Сидней 21–6): окно с a > b идёт через 00:00 UTC, простое
        # сравнение a <= h < b на нём всегда ложно — рынок считался бы закрытым круглые сутки.
        _open = (a <= h < b) if a < b else (h >= a or h < b)
        if _open:
            b = b if a < b else (b + 24)
            h_eff = h if (a < b - 24 or h >= a) else (h + 24)
            left = round(b - h_eff, 1)
            state = ("открылась" if h_eff - a < SOON_H
                     else ("скоро закроется" if left <= SOON_H else "идёт"))
...
def bg_note(row: dict | None = None) -> list:
    ...
    if not r:
        return []
    out: list = []
    # ОДИН ПРИБОР ВМЕСТО ДВУХ (13.09, владелец: «давай один прибор сделаем из этих двух… название
    # — монеты к медиане»). Ширина (сколько растёт) и медиана (насколько) — две стороны одного
    # вопроса, и ценно как раз их расхождение: растёт половина, а медиана −1% значит, что растут
    # по чуть-чуть, а падают сильно. Состояние берём по медиане, оба числа — в одной строке.
    br = r.get("breadth") or {}
    med = ((r.get("risk_on") or {}).get("median_pct"))
    if br.get("n") or med is not None:
        share = (br["up"] / br["n"]) if br.get("n") else None
        if med is not None:
        ...
        num = []
        if med is not None:
            num.append(f"{med:+.2f}% медиана хода")
        if br.get("n"):
            num.append(f"растёт {br['up']} из {br['n']}")
        out.append(["монеты к медиане", state, " · ".join(num)])
