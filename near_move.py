...
def _today_bars(sym_usdt: str) -> dict | None:
    ...
    for i, (r, x) in enumerate(zip(full, dl)):
        ...
        bubbles.append({
            ...
            # МЕСТО В ДНЕ МОЖЕТ НЕ ПОСЧИТАТЬСЯ (14.09): при нулевом размахе дня _pos = None, и
            # сравнение с числом роняло весь блок «Близкие» — TypeError на NoneType <= int.
            # Без места фраза строится по стороне и по тому, что стало с интересом.
            "say": (
                "набрали внизу" if (x > 0 and _pos is not None and _pos <= 40 and _fill == "набрали") else
                "об покупки закрылись" if (x > 0 and _fill == "закрылись") else
                "покупку приняли наверху" if (x > 0 and _pos is not None and _pos >= 70) else
                "вынесли продавца" if (x < 0 and _pos is not None and _pos <= 40 and _fill == "закрылись") else
                "продажу приняли внизу" if (x < 0 and _pos is not None and _pos <= 40 and _fill == "набрали") else
                "продали наверху" if (x < 0 and _pos is not None and _pos >= 70) else
