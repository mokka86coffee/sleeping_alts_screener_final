...
ВЫХОД — событие, не цена (правило 07.09 «выход только рука ушла»): слом вортекса ВНИЗ от максимума разрыва
≥ BRK_MIN_VX ИЛИ слом клингера вниз после того, как цена была выше входа. Ценового стопа нет — размер на бумаге
условный 1.0, риск считается по итогам.
...
def evaluate(sym: str, F: dict, px_now: float | None) -> dict:
    ...
    r = {"sym": sym, "t": t, "vx_break": bv[-1], "kl_break": (bk[-1] if bk else 0), "gap": round(gv[-1], 3),
         "oi_up": oi_up, "fund": fund, "px": px_now, "signal": None, "why": ""}
...
def main() -> int:
    ...
    for sym in syms + [s for s in list(state["open"].keys()) if s not in syms]:   # открытые смотрим всегда
        ...
        line = f"paper_fast: {sym} · " + (ev["signal"] or "тихо") + (" · " + ev["why"] if ev["why"] else "")
        print(line)
        if ev["signal"] == "entry" and not pos and sym in syms:
            state["open"][sym] = {"t": ev["t"], "px": ev["px"], "why": ev["why"], "opened_at": now}
            opened.append(dict(ev, kind="entry"))
        elif ev["signal"] == "exit" and pos:
            # клингер закрывает только если цена была выше входа: иначе это не «после вершины», а шум у дна
            if "клингер" in ev["why"] and pos.get("px") and ev["px"] and ev["px"] <= pos["px"]:
                continue
            res = (ev["px"] / pos["px"] - 1) * 100 if (pos.get("px") and ev["px"]) else None
            closed.append(dict(ev, kind="exit", entry_px=pos.get("px"), entry_t=pos.get("t"), result_pct=(round(res, 2) if res is not None else None)))
            del state["open"][sym]
    if a.write:
