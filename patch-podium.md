...
# 1 · `render/brief.py` — контейнер под текстом

Сцена живёт внутри сводки, после текста: сначала что происходит
сейчас, потом кто в журнале и что с ними. Обратный порядок заставлял
бы читать историю до текущего состояния.
...
**Ширина сцены.** `.obf-podium` выходит за колонку текста
отрицательным отступом слева. Если разметка сводки центрируется иначе,
чем я предполагаю по `.obf-in{width:min(660px,88vw)}`, сцена уедет — и
чинится это одной строкой отступа.
