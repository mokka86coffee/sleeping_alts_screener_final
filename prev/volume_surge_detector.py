...
from core_binance import K_QUOTE_VOLUME, K_OPEN, K_CLOSE, klines_1d, series
...
class VolumeSurgeSignal:
    ...
    current_vol_usd: float = 0.0      # фактически набранный объём текущих суток
    projected_vol_usd: float = 0.0    # объём, экстраполированный на конец суток
...
def detect_volume_surge(symbol: str) -> VolumeSurgeSignal:
    ...
    # Формулировка честно разделяет факт и прогноз: до закрытия суток
    # это темп набора объёма, а не итоговая цифра
    if projected:
        vol_phrase = (
            f"темп объёма ${_fmt_usd(current_vol)} в пересчёте на сутки "
            f"(набрано ${_fmt_usd(raw_vol)} за {fraction * 100:.0f}% дня)"
        )
    else:
        vol_phrase = f"дневной объём ${_fmt_usd(raw_vol)}"
