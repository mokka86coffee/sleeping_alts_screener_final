    ...
    if not s["items"]:
        ...
        for c in sorted(s["items"], key=lambda x: -x.score):
            ...
            rows += (
                ...
                f'<td class="tb-s">{_tick(c)}</td>'
                f'<td class="tb-sc"><i style="--p:{min(int(c.score), 100)}"></i>'
                f'{c.score}</td>'
                f'<td class="{cls}">{ch:+.1f}%</td>'
                f'<td>{_num(c, "rvol_1h"):.1f}×</td>'
                f'<td>{f"1:{rr:.1f}" if rr else "—"}</td>'
                f'<td class="tb-ph">'
                f'{esc(str((c.phase or {}).get("label", "—")).lower())}</td></tr>'
            )
        body = (
            '<table class="tb"><thead><tr>'
            '<th>монета</th><th>score</th><th>24h</th>'
            '<th>rvol</th><th>r:r</th><th>фаза</th>'
...
<div class="pane" data-pane="{esc(s['id'])}">
  <div class="pane-hd">
    ...
    <span class="pane-t">{esc(s['label'].lower())}</span>
