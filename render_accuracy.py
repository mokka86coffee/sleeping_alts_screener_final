    ...
    for k, t in enumerate(runs):
        ...
        if k:
            for sym in by_run[runs[k - 1]]:
                # ВЫПАДЕНИЕ — СТУПЕНЬЮ В ПУТИ, НА СВОЁМ МЕСТЕ ПО ВРЕМЕНИ (11.09, владелец, MINA: «04:38»
                # красным стояло в конце строки после «16:38»). Раньше писалось только первое
                # выпадение отдельным полем и рисовалось последним; теперь каждое выпадение —
                # ступень без места (place=None) в хронологии, возврат в очередь открывает новую ступень.
                if sym not in by_run[t] and sym in d:
