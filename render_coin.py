...
def _fast_events(days: int | None = None, only: list | None = None) -> dict:
    ...
    # ТОЛЬКО НУЖНЫЕ МОНЕТЫ (18.09): сводке наблюдения (watch_brief) нужны быстрые по двум-трём монетам,
    # а не по всей доске. Без параметра поведение прежнее: вся папка архива.
    _only = {str(x).upper().replace("USDT", "") for x in (only or [])} or None
    out: dict = {}
    ...
    for p in d.glob("*.jsonl"):
        if _only is not None and p.stem.upper() not in _only:
            continue
        rows = []
        try:
        ...
        for i, (t, r) in enumerate(rows):
            ...
            if o:
                prev = o
        # РЯДЫ ДЛЯ ПЛИТЫ БЫСТРЫХ (14.09 вечер): vx30 — настоящий вортекс по размаху бара (VM+ = |h − l_prev|,
    ...
    for _sym, _ser in _fs.items():
        ...
        _dst = out.setdefault(_sym, {"bubbles": [], "end": [], "force": [], "entry": [], "hedge": [], "start": [],
                                     "vx30": [], "kl30": [], "fund30": [], "oi30": [], "vol30": []})
        for _k in ("vx30", "kl30", "fund30", "oi30", "vol30"):
            if _ser.get(_k):
...
def render_coin(stars: list[dict], market: dict) -> str:
    ...
    # до открытия следующей сессии — час, после открытия — было полтора часа, стало два с половиной.
    try:
        from core_config import COIN_MOMENT_PRE_MIN, COIN_MOMENT_POST_MIN
    except ImportError:
        COIN_MOMENT_PRE_MIN, COIN_MOMENT_POST_MIN = 60, 150
    # СТРЕЛКА ВОРТЕКСА И ЗНАК КЛИНГЕРА (18.09): окно перевеса — полтора-два часа; клингер вниз — после
    # стольких баров роста подряд
    try:
...
/* ЛУЧИ ПОД ПЛИТОЙ БЫСТРЫХ (15.09, владелец: «лучи тоже, как у «держать», что-то красивое, светлое»): от земли
   плиты вниз веером расходятся тёплые светлые лучи, у основания каждого искра; дышат, как лучи вердикта. */
.mini.fast .fray{position:absolute;top:calc(100% - 30px);width:1px;height:150px;pointer-events:none;transform-origin:50% 0;
  background:linear-gradient(180deg,rgba(255,240,210,.7),rgba(255,236,200,.25) 40%,rgba(255,236,200,0));
  animation:fray var(--rd,6s) ease-in-out var(--rw,0s) infinite alternate}
.mini.fast .fray::before{content:"";position:absolute;left:-1.5px;top:-1px;width:3px;height:3px;border-radius:50%;background:#fff;box-shadow:0 0 6px #ffe9b0,0 0 12px rgba(255,217,138,.7)}
.mini.fast .fglow{position:absolute;left:40px;right:40px;top:calc(100% - 44px);height:40px;border-radius:50%;pointer-events:none;
  background:radial-gradient(rgba(255,226,168,.42),rgba(255,226,168,0) 70%);filter:blur(12px);animation:railhalo 6s ease-in-out infinite}
@keyframes fray{0%{transform:rotate(var(--ra,0deg)) scaleY(.55);opacity:.22}100%{transform:rotate(var(--ra,0deg)) scaleY(1);opacity:.6}}
.mini.fast .fray,.mini.fast .fglow{z-index:0}
.mini.fast .fread,.mini.fast .fdemo,.mini.fast .ftip{z-index:2}   /* тексты поверх лучей, лучи — фон под ними, как у вердикта */
.mini.fast svg text.fF{font-family:var(--f-cap);font-size:7.5px;font-weight:600;filter:drop-shadow(0 0 3px var(--c))}
...
.mini.fast svg g.turn{transform-box:fill-box;transform-origin:center;animation:ringp 3s ease-in-out infinite}
...
.railbox::before{content:"";position:absolute;left:-40px;right:-40px;top:-26px;bottom:-22px;border-radius:50%;pointer-events:none;z-index:-1;
  background:radial-gradient(ellipse at center,rgba(80,140,255,.30),rgba(80,140,255,.10) 55%,rgba(80,140,255,0) 75%);filter:blur(14px);
  animation:railhalo 5s ease-in-out infinite}
/* ЗАТЕМНЕНИЕ под рядом (владелец, 14.09 вечер: «сливаются с отражением графика»): тёмное пятно под голубым
   ореолом, мягкие края — гасит отражение плиты, буквы читаются на своём фоне */
.rail::before{content:"";position:absolute;left:-46px;right:-46px;top:-22px;bottom:-20px;border-radius:50%;pointer-events:none;z-index:-2;
  background:radial-gradient(ellipse at center,rgba(2,10,8,.92),rgba(2,10,8,.75) 55%,rgba(2,10,8,0) 78%);filter:blur(8px)}
@keyframes railhalo{0%,100%{opacity:.75}50%{opacity:1}}
/* БЕГУЩИЙ СВЕТ (владелец, 14.09 вечер: «пусти по ним светящуюся штуку взад-вперёд»): тонкая линия под
...
/* РЕЙКА БЕЗ ПЛАШЕК (14.09 вечер, владелец: «синеватыми по аналогии с «держать», анимацию подобную,
   ...
   тот же холодный белый #eaf4ff и голубое сияние; сила сигнала (--st 0..1) задаёт и яркость, и радиус
   свечения, и размах дыхания. Молчащий — серые буквы без света, прочерк вместо стрелки. Один ряд. */
.rail{position:relative;left:auto;bottom:auto;transform:none;display:flex;flex-direction:row;gap:11px;align-items:flex-end;white-space:nowrap}
...
.rail i.on u{color:#eaf4ff;
  text-shadow:0 0 calc(3px + 9px * var(--st)) rgba(150,200,255,calc(.35 + .6 * var(--st))),0 0 calc(10px + 22px * var(--st)) rgba(120,170,255,calc(.15 + .5 * var(--st)));
  animation:railbreath calc(4.2s - 1.6s * var(--st)) ease-in-out 1.2s infinite}
.rail em{font-family:var(--f-cap);font-size:6.5px;letter-spacing:.14em;font-style:normal;color:#5f7f98}
.rail i.on em{color:#8fb8d8}
.rail i.on em b{color:#bfe0ff;font-weight:400}
.rail i svg{position:static;width:11px;height:11px;flex:0 0 11px;overflow:visible}
.rail i.on svg path{filter:drop-shadow(0 0 calc(2px + 3px * var(--st)) rgba(150,200,255,.7))}
.rail i.on.up svg path{animation:railbr-up 2.4s ease-in-out infinite}
.rail i.on.dn svg path{animation:railbr-dn 2.4s ease-in-out infinite}
@keyframes railin{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:none}}
@keyframes railin-off{from{opacity:0;transform:translateY(6px)}to{opacity:.7;transform:none}}
@keyframes railbreath{0%,100%{opacity:calc(.7 + .3 * var(--st))}50%{opacity:1}}
@keyframes railbr-up{0%,100%{transform:translateY(0)}50%{transform:translateY(-1.6px)}}
@keyframes railbr-dn{0%,100%{transform:translateY(0)}50%{transform:translateY(1.6px)}}
</style>
...
(function () {
  ...
  function groups(s) {
    ...
    // ТРОЙКА: ИНТЕРЕС + ОБОРОТ + КАПА (12.09, владелец: «рост плеча — это не дальнейший рост»).
    // Все три вверх — приходят новые деньги; плечо вверх при падающем обороте — набивка внутри
    // прежнего объёма, она кончается выносом набившихся.
    if (has(s.oiChg) && has(s.v1d)) {
      var _moneyIn = (+s.v1d >= 1) && (!has(s.capChgPct) || +s.capChgPct > 0);
      // Четыре состояния (12.09, владелец): у дна без оборота — тихий откуп; высоко от дна без
      // оборота — раздача; плечо и оборот есть, а капа стоит — раздают об толпу; все три растут —
      // продолжение роста, потолок по весу около миллиарда капитализации.
      var _up = has(s.up) ? +s.up : null, _capUp = has(s.capChgPct) && +s.capChgPct > 0, _volUp = +s.v1d >= 1;
      var _state = +s.oiChg <= 0 ? '' :
        _volUp && _capUp ? ' — продолжение роста' :
        _volUp ? ' — РАЗДАЮТ ОБ ТОЛПУ: оборот есть, цену не двигает' :
        (_up !== null && _up >= 150) ? ' — РАЗДАЧА: высоко от дна, оборота нет' :
        ' — тихий откуп у дна, накопление';
      pr.push(['деньги', 'плечо ' + (+s.oiChg > 0 ? '+' : '') + Math.round(+s.oiChg) + '%'
        + ' · оборот ×' + (+s.v1d).toFixed(1) + (has(s.capChgPct) ? ' · капа ' + (s.capChgPct > 0 ? '+' : '') + (+s.capChgPct).toFixed(1) + '%' : '')
        + _state]);
    }
    ...
    // БЫСТРЫЙ ВИХРЬ — ХЕДЖ (11.09): рядом с силой, из near_move.json, в балл не идёт. Одна строка
    // «против» по форме хода (парабола: продавцы под максимумом дня; лестница: сторона сменилась),
  ...
  function build(tick) {
    ...
    (function () {
      if (!_bias) return;
      var zs = (s.liqZones || []).filter(function (z) { var mid = (z.lo + z.hi) / 2; return _bias === 'down' ? mid < _pxNow : mid > _pxNow; });
      if (!zs.length) return;
    ...
    RL.forEach(function (r) {
      ...
      g += '<line x1="' + r.x1 + '" y1="' + f(r.y) + '" x2="' + (X1 + 30) + '" y2="' + f(r.y) + '" stroke="' + (r.liq ? (r.op > .6 ? r.col : '#fff1cc') : r.col) + '" stroke-width="' + r.w + '" opacity="' + r.op + '" stroke-dasharray="' + r.dash + '"/>';
      ...
      var isDir = !!(dirPl && r.hot && Math.abs(r.mid - dirPl.mid) < 1e-12);
      if (isDir) {
    ...
    if (_hrs.length > 48) (function () {
      ...
      (F.entry || []).forEach(function (e) { _EV.push({ t: e.t, px: +e.px, dir: 'up', kind: 'vx', gap: +e.gap, col: OR, op: .95, short: 'вортекс: покупатели взяли сторону', tip: 'вортекс 30м: покупатели взяли сторону · ' + hhmm(e.t) + ' · разрыв ' + (+e.gap).toFixed(2) }); });
      ...
      (function () { var K = s.klinger30; if (!K) return;
        var dn = K.crossDn || (K.above && (+K.gapX || 1) <= 0.25 && K.narrowing), upc = K.crossUp;
        var ago = dn ? K.dnAgo : K.upAgo; if (!(K.crossUp || K.crossDn) && !dn) return;
        var t = _tEnd - ((+ago || 0) * 18e5), b = null; for (var q = 0; q < _hrs.length; q++) { if (_hrs[q][0] <= t) b = _hrs[q]; else break; }
        var str = (K.crossUp || K.crossDn) ? 1 : .5;
        _EV.push({ t: t, px: b ? +b[4] : +s.px, dir: dn ? 'down' : 'up', kind: 'kl', col: dn ? RD : GR, op: str, str: str,
          short: K.crossDn ? 'клингер: крест вниз' : K.crossUp ? 'клингер: крест вверх' : 'клингер сходится — крест близко',
          tip: 'клингер 30м · ' + (K.crossDn ? 'крест вниз' : K.crossUp ? 'крест вверх' : 'сходится, крест близко') + ' · ' + hhmm(t) + ' · KVO ' + f(K.kvo) + ' при сигнале ' + f(K.sig) }); })();
      ...
      (function () {
        ...
        _hrs.forEach(function (b, i) {
          ...
          _EV.push({ t: b[0], px: +b[4], dir: answered ? (d > 0 ? 'up' : 'down') : (d > 0 ? 'down' : 'up'),
            kind: 'sess', col: answered ? (d > 0 ? GR : RD) : OR, op: .9, answered: answered, volx: nrm ? vol / nrm : 1,
    ...
    //    «усиливается» (разрыв растёт три бара подряд) — свечение. Клингер и вортекс — самые ранние
    //    индикаторы разворота, поэтому развороты на этой плите главное, ровные бары прорежены.
    //    Ряды: D.fast[sym].vx30 = [[t, vi_plus, vi_minus]], D.fast[sym].kl30 = [[t, kvo, sig]] — пишет
    //    прогон. Нет рядов — ЗАМЕНИТЕЛЬ по закрытиям и обороту часовой ленты с пометкой «подставлено».
    (function () {
      var _S = String(s.coin || (String(s.t).toUpperCase() + 'USDT')).toUpperCase();
      var hrs = ((OIT[_S] || {}).hours || []).filter(function (b) { return +b[4] > 0; });
      if (hrs.length < 24) return;
      var tEnd = hrs[hrs.length - 1][0], tBeg = tEnd - 3 * 864e5;
      var F = (D.fast || {})[_S] || {}, demo = [];
      var VX = F.vx30, KL = F.kl30;
      if (!VX || VX.length < 8) { demo.push('вортекс по закрытиям'); VX = []; var N = 14;
        for (var i = 1; i < hrs.length; i++) { var up = 0, dn = 0, tr = 0;
          for (var j = Math.max(1, i - N + 1); j <= i; j++) { var dd = +hrs[j][4] - +hrs[j - 1][4]; up += Math.max(0, dd); dn += Math.max(0, -dd); tr += Math.abs(dd); }
          VX.push([hrs[i][0], tr ? up / tr * 1.4 + 0.3 : 1, tr ? dn / tr * 1.4 + 0.3 : 1]); } }
      if (!KL || KL.length < 8) { demo.push('клингер по закрытиям и обороту'); KL = [];
        var e12 = null, e26 = null, e9 = null;
        for (var i2 = 1; i2 < hrs.length; i2++) { var vf = (+hrs[i2][5] || 0) * ((+hrs[i2][4] >= +hrs[i2 - 1][4]) ? 1 : -1);
          e12 = e12 === null ? vf : e12 + (vf - e12) * 2 / 13; e26 = e26 === null ? vf : e26 + (vf - e26) * 2 / 27;
          var kvo = e12 - e26; e9 = e9 === null ? kvo : e9 + (kvo - e9) * 2 / 10; KL.push([hrs[i2][0], kvo, e9]); } }
      VX = VX.filter(function (r) { return r[0] >= tBeg && r[0] <= tEnd; }); KL = KL.filter(function (r) { return r[0] >= tBeg && r[0] <= tEnd; });
      ...
      // полутора шагов — дыра: на цене пунктир, на лентах штриховка, в шапке «архив N из M баров».
      var steps = []; for (var q0 = 1; q0 < win.length; q0++) steps.push(win[q0][0] - win[q0 - 1][0]);
      steps.sort(function (a, b) { return a - b; }); var STEP = steps.length ? steps[Math.floor(steps.length / 2)] : 36e5;
      var GAPS = [], missing = 0;
      for (var q1 = 1; q1 < win.length; q1++) { var dt = win[q1][0] - win[q1 - 1][0]; if (dt > STEP * 1.5) { GAPS.push([win[q1 - 1][0], win[q1][0]]); missing += Math.round(dt / STEP) - 1; } }
      var expected = win.length + missing;
      var dp = '', dpg = '', Ln = 0;
      win.forEach(function (b, i) { var x = X(b[0]).toFixed(1), y = Y(+b[4]).toFixed(1);
        if (!i) { dp += 'M' + x + ',' + y; return; }
        var hole = (b[0] - win[i - 1][0]) > STEP * 1.5;
        if (hole) { dpg += 'M' + X(win[i - 1][0]).toFixed(1) + ',' + Y(+win[i - 1][4]).toFixed(1) + ' L' + x + ',' + y + ' '; dp += ' M' + x + ',' + y; }
      ...
      var STATE = {}, BREAKS = [];
      function hatch(y, XF) { var o = ''; XF = XF || X; GAPS.forEach(function (gp) { var x0 = XF(gp[0]), x1 = XF(gp[1]);
        o += '<rect x="' + x0.toFixed(1) + '" y="' + (y - 3) + '" width="' + (x1 - x0).toFixed(1) + '" height="6" fill="url(#fhatch)" data-tip="' + esc('нет данных · ' + hhmm(gp[0]) + ' → ' + hhmm(gp[1])) + '"/>'; }); return o; }
...
;   // подпись «интерес» снята (владелец 15.09) — линия синяя, узнаётся без слова
        ...
        // не выстраивать частокол; яркость — по глубине.
        var FUND_NEG = -0.5, FUND_POS = 0.1, FD = F.fund30 || [], prevSign = 0;
        FD.forEach(function (r) { if (r[0] < tBeg || r[0] > tEnd) return; var fv = +r[1];
          var sign = fv <= FUND_NEG ? -1 : fv >= FUND_POS ? 1 : 0;
          if (sign === prevSign) return; prevSign = sign; if (!sign) return;
          var near = null; OI.forEach(function (q) { if (q[0] <= r[0]) near = q; }); if (!near) return;
          var tipF = 'фандинг ' + fv.toFixed(2) + '% · ' + hhmm(r[0]) + (sign < 0 ? ' · шорты платят' : ' · платят лонги');
          var colF = sign < 0 ? RD : GR, opF = Math.min(1, .55 + Math.abs(fv) / (sign < 0 ? 2 : 0.4)).toFixed(2);
          o += '<text class="fF" x="' + X(r[0]).toFixed(1) + '" y="' + (Y2(near[1]) + 11).toFixed(1) + '" text-anchor="middle" fill="' + colF + '" opacity="' + opF + '" style="--c:' + colF + '" data-tip="' + esc(tipF) + '">F</text>'
            + '<rect class="hit" data-tip="' + esc(tipF) + '" x="' + (X(r[0]) - 4).toFixed(1) + '" y="' + (Y2(near[1]) + 3).toFixed(1) + '" width="8" height="11" fill="transparent"/>'; });
      ...
      // сила. Стрелки остаются ТОЛЬКО на барах слома после экстремума; последний слом — крупно, с кольцом и
      // временем, прежние — мелкими стрелками. rows = [[t, a, b]], a — покупатели/KVO, b — продавцы/сигнал.
      function row(rows, y, name) {
        ...
        // ПОРОГ ЭКСТРЕМУМА (15.09, по LAB в штиле: сутки в коридоре 6% давали по десять «сломов» в день — экстремумы
        // крошечные). Слом считается только от экстремума, который что-то значит: у вортекса |разрыв| ≥ BRK_MIN_VX
        // (0.25), у клингера — |разрыв| ≥ BRK_MIN_KL от размаха разрыва за окно (0.25). Мелкие волны — не сломы.
        var EXT = 6, BRK_MIN_VX = 0.25, BRK_MIN_KL = 0.25;   // 0.25 по LAB: 12 сломов за три дня → 6, и каждый — на настоящем отскоке
        var VX_WIN = (D.cfg && D.cfg.vxWin) || 4, KL_UP_BARS = (D.cfg && D.cfg.klUp) || 3;   // окно перевеса (18.09, «полтора-два часа») и баров роста клингера до разворота
        var big = name === 'клингер' ? BRK_MIN_KL * mx : BRK_MIN_VX;
        ...
        var brk = rows.map(function (r, i) {
          if (i < EXT + 1) return 0;
          if (gaps[i] < gaps[i - 1] && isMax(gaps, i - 1) && gaps[i - 1] >= big) return -1;
          if (gaps[i] > gaps[i - 1] && isMin(gaps, i - 1) && gaps[i - 1] <= -big) return 1;
          return 0; });
        var turnIdx = []; brk.forEach(function (v, i) { if (v) turnIdx.push(i); });
        var lastT = turnIdx[turnIdx.length - 1];
        var bwR = (RW - 24) / Math.max(1, (tEnd - tBeg) / STEP);   // ширина бара по шагу архива, не по соседям
        ...
        // ДВИЖЕНИЕ ПЕРЕВЕСА (18.09, владелец его словами): вортекс — «если каждый следующий бар продавцы давят
        // сильнее — стрелка вниз, слабеют — зелёная вверх»; окно полтора-два часа (VX_WIN баров). Яркость у обеих
        // сторон ОДНА мерка — как быстро едет перевес («почему они по разному???»); скорость сравнивается с обычным
        ...
        if (name === 'клингер') {
          for (var c = n - 1; c >= 1; c--) {
            var a1 = +rows[c][1] - +rows[c][2], a0 = +rows[c - 1][1] - +rows[c - 1][2];
            if (a1 > 0 && a0 <= 0) { BREAKS.push({ t: rows[c][0], side: 1, name: name, cross: true }); break; }
            if (a1 < 0 && a0 >= 0) { BREAKS.push({ t: rows[c][0], side: -1, name: name, cross: true }); break; }
          }
        }
      ...
      // следующей сессии, межсессионье»). Открытия в UTC как в near_move: Сидней 21, Токио 0, Лондон 7, Нью-Йорк 13.
      // Стык — час до открытия и полчаса после: светлая полоса на всю высоту; линия открытия — цветом сессии;
      // подписи — только за последние сутки, раньше — засечки. Справа вверху — до следующей сессии в местных
      // часах. Рисуется под ценой и лентами.
      var SESS = [[21, 'Сидней', '#ffd98a', 'СД'], [0, 'Токио', '#ff9fb8', 'ТК'], [7, 'Лондон', '#7fc0ff', 'ЛД'], [13, 'Нью-Йорк', '#ffb27a', 'НЙ']];
      var sessG = '', dayMs = 864e5, d0 = Math.floor(tBeg / dayMs) * dayMs, NEXTSESS = '';
      ...
      (function () { var now = Date.now(), best = null;
        SESS.forEach(function (se) { var dn = Math.floor(now / dayMs) * dayMs + se[0] * 36e5; if (dn <= now) dn += dayMs; if (!best || dn < best[0]) best = [dn, se[1], se[2]]; });
        if (!best) return; var m = Math.round((best[0] - now) / 6e4), hh = Math.floor(m / 60), mm = m % 60;
        NEXTSESS = '<b style="color:' + best[2] + '">до ' + esc(best[1]) + ' ' + hh + ':' + pad(mm) + (m <= 60 ? ' · стык' : '') + '</b>'; })();
      // ПОДХВАТИЛА ЛИ НОВАЯ СЕССИЯ (15.09, владелец: «добавить разбор — поддержала ли новая сессия»). То же правило,
      // что в near_move с 12.09: на стыке крупный отдаёт и смотрит, кто примет. Подхват — три условия вместе: оборот
      // двух первых баров выше нормы ЭТОЙ сессии, дельта в плюс, интерес вырос — новые руки пришли с плечом. Один
      // оборот без плеча — перекладка. Ряд D.fast[sym].vol30 = [[t, оборот, дельта]] пишет прогон; нет ряда —
      // считается по часовой ленте (оборот бара, дельты нет — тогда два условия из трёх, помечено «без дельты»).
      // КОНЕЦ ДАННЫХ ДЛЯ СТЫКОВ (16.09, владелец: «в звёздах уже Токио и подхватил Сидней, а в карточке до сих пор
      // Нью-Йорк подхватил»). Окно плиты кончается на последнем часовом баре ленты, а лента обновляется реже архива
      // получасовок: открытие Сиднея в неё не попадало, и последним стыком оставался Нью-Йорк. Стыки считаются до
      // конца архива (tJ); кружок на плите рисуется, только если открытие внутри окна.
      var tJ = tEnd;
      [F.vol30, F.oi30].forEach(function (a) { if (a && a.length) tJ = Math.max(tJ, +a[a.length - 1][0]); });
      var VOL = (F.vol30 || []).filter(function (r) { return r[0] >= tBeg && r[0] <= tJ; }), volFromHours = false;
      if (VOL.length < 8) { volFromHours = true; VOL = win.map(function (b) { return [b[0], +b[5] || 0, null]; }); tJ = tEnd; }
      var OIS = (F.oi30 || []).filter(function (r) { return r[0] >= tBeg && r[0] <= tJ; });
      function atOrBefore(arr, t) { var q = null; for (var i = 0; i < arr.length; i++) { if (arr[i][0] <= t) q = arr[i]; else break; } return q; }
      function sessOf(h) { return h >= 21 || h < 6 ? 'Сидней' : h < 7 ? 'Токио' : h < 13 ? (h < 9 ? 'Токио' : 'Лондон') : h < 16 ? 'Лондон' : 'Нью-Йорк'; }
      ...
      for (var dj = d0 - dayMs; dj <= tJ; dj += dayMs) SESS.forEach(function (se) {
        ...
        // РАННЕЕ ЧТЕНИЕ (19.09, владелец: «если есть подтверждение на первых барах, ждать уже не надо»):
        // считаем после КАЖДОГО закрытого бара; все три условия сошлись — подхватил уже по этому бару, дальше только
        // уточняется; не сошлись — «проба идёт», ждём следующий; окончательное «не подхватил» — только по PICK_N.
        var have = Math.min(PICK_N, Math.floor((tJ - t) / STEPn) + 1);
        if (have < 1) { JOINS.push({ t: t, ok: null, se: se, why: se[1] + ' ' + hhmm(t) + ' · подхват ещё не прочитан — бар открытия не закрыт', short: 'бар открытия не закрыт' }); return; }
        var b0 = atOrBefore(VOL, t);
        if (!b0 || b0[0] < t - STEPn / 2) { JOINS.push({ t: t, ok: null, se: se, why: se[1] + ' ' + hhmm(t) + ' · бара открытия нет в архиве — подхват не прочитан', short: 'бара открытия нет в архиве' }); return; }
        var oa = atOrBefore(OIS, t - STEPn), pa = atOrBefore(win.map(function (b) { return [b[0], +b[4]]; }), t - STEPn);
        var ok = false, used = 0, volx = null, dl = null, oiCh = null, pxCh = null, ob = null, tLast = t;
        for (var k = 1; k <= have; k++) {
          var bars = []; for (var q = 0; q < k; q++) { var bq = atOrBefore(VOL, t + q * STEPn); if (!bq || bq[0] < t + q * STEPn - STEPn / 2) { bars = null; break; } bars.push(bq); }
          if (!bars) break;
          tLast = t + (k - 1) * STEPn; used = k;
          var vol = bars.reduce(function (a, b) { return a + b[1]; }, 0), nrm = (normS[se[1]] || 0) * k; volx = nrm ? vol / nrm : null;
          dl = bars.some(function (b) { return b[2] === null; }) ? null : bars.reduce(function (a, b) { return a + b[2]; }, 0);
        ...
        if (!ok && have < PICK_N) {                                             // ответа пока нет, окно не вышло
          JOINS.push({ t: t, ok: null, se: se, why: se[1] + ' ' + hhmm(t) + ' · проба идёт — по ' + used + ' бар' + (used === 1 ? 'у' : 'ам') + ' пока не подхватил, ждём ещё ' + (PICK_N - used) + nums,
                       short: 'проба идёт · по ' + used + ' бар' + (used === 1 ? 'у' : 'ам') + ' пока нет' + nums.replace(/ · цена[^·]*$/, '') }); return; }
        // подтверждение на следующем баре после решения: плечо растёт дальше и оборот не сдулся
        var conf = '', tc = tLast + STEPn;
        if (tc <= tJ) {
      ...
      (function () {
        var NP = (((NEAR[_S] || {}).today) || {}).sess_pickup; if (!NP || !NP.at) return;
        var tn = Date.parse(NP.at); if (isNaN(tn)) return;
        var se = null; SESS.forEach(function (q) { if (q[1] === NP.session) se = q; }); if (!se) return;
        var sh = (NP.vol_x != null ? 'оборот <b>×' + (+NP.vol_x).toFixed(1) + '</b>' : '') + (NP.oi_chg_pct != null ? ' · интерес <b>' + (NP.oi_chg_pct > 0 ? '+' : '') + (+NP.oi_chg_pct).toFixed(0) + '%</b>' : '');
        var j = null; JOINS.forEach(function (q) { if (q.t === tn) j = q; });
        if (!j) { j = { t: tn, se: se }; JOINS.push(j); }
        j.ok = !!NP.pickup; j.why = se[1] + ' ' + hhmm(tn) + ' · ' + NP.why + (sh ? ' · ' + sh.replace(/<\/?b>/g, '') : ''); if (sh) j.short = sh;
      })();
      JOINS.sort(function (a, b) { return a.t - b.t; });
      var joinG = '';
      JOINS.forEach(function (j) { if (j.t > tEnd || j.ok === null) return; var x = X(j.t).toFixed(1), y = GY - 6;
        joinG += j.ok ? '<circle cx="' + x + '" cy="' + y + '" r="2.6" fill="' + GR + '" class="jn" data-tip="' + esc(j.why) + '"/>'
                      : '<circle cx="' + x + '" cy="' + y + '" r="2.4" fill="none" stroke="' + RD + '" stroke-width="1" class="jn" data-tip="' + esc(j.why) + '"/>'; });
      if (JOINS.length) { var lj = JOINS[JOINS.length - 1];
        STATE['стык'] = '<b style="color:' + lj.se[2] + '">' + esc(lj.se[1]) + ' ' + hhmm(lj.t) + '</b> ' + (lj.ok === null ? 'подхват <b>не прочитан</b>' : lj.ok ? '<b>подхватил</b>' : '<b>не подхватил</b>') + ' · ' + lj.short; }
      g = '<defs><filter id="fglow" x="-60%" y="-60%" width="220%" height="220%"><feGaussianBlur stdDeviation="2.2"/></filter>'
        + '<pattern id="fhatch" width="4" height="4" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><line x1="0" y1="0" x2="0" y2="4" stroke="rgba(233,255,244,.28)" stroke-width="1"/></pattern></defs>' + sessG + g + joinG
      ...
      // с вёрсткой что-то» — стрелки лежали одна на другой).
      BREAKS.sort(function (a, b) { return a.t - b.t; });
      // ВСЕГДА НАД ГРАФИКОМ, ПОДПИСЬ — ОДНА БУКВА (владелец, 14.09 вечер: «стрелки над графиком, В и К достаточно,
      // подсвети буквы»): стрелка над точкой цены своего бара, направление — треугольником; два слома на одном
      // баре — второй выше первого; буква В/К светится цветом слома, полное имя и время — в подсказке.
      var placed = [];
      function bkTip(bk) {
        var up = bk.side > 0;
        if (!bk.move) return bk.name + ' · слом ' + (up ? 'вверх' : 'вниз') + ' · ' + hhmm(bk.t);
        if (bk.cross) return bk.name + ' · крест с сигнальной ' + (up ? 'вверх' : 'вниз') + ' · ' + hhmm(bk.t);
        var what = bk.name === 'клингер' ? 'рос и развернулся вниз'
                                          : ((up ? 'продавцы слабеют' : 'продавцы давят сильнее')
                                             + (bk.spd != null ? ' · скорость ' + Math.abs(bk.spd).toFixed(3) + ' за бар' : '')
                                             + ' · сила ' + Math.round((bk.st || 0) * 100) + '%');
        return bk.name + ' · ' + what + (bk.lastBrk ? ' · последний слом ' + (bk.lastBrk.side > 0 ? 'вверх' : 'вниз') + ' ' + hhmm(bk.lastBrk.t) : '');
      }
      BREAKS.forEach(function (bk) {
        var bar = null; for (var q = 0; q < win.length; q++) { if (win[q][0] <= bk.t) bar = win[q]; else break; }
        if (!bar) return;
        var x = X(bk.t), yp = Y(+bar[4]), up = bk.side > 0, col = up ? GR : RD, sz = 5;
        var op = (bk.move && !bk.cross) ? (0.3 + 0.7 * Math.pow(bk.st || 0, 0.8)).toFixed(2) : '1';
        var lvl = placed.filter(function (o) { return Math.abs(o.x - x) < 14; }).length;
        var ya = Math.max(14, yp - 13 - lvl * 19);
        placed.push({ x: x });
        var tri = up ? 'M' + (x - sz).toFixed(1) + ',' + (ya + sz * .8).toFixed(1) + ' h' + (2 * sz) + ' l-' + sz + ',-' + (sz * 1.6) + ' z'
                     : 'M' + (x - sz).toFixed(1) + ',' + (ya - sz * .8).toFixed(1) + ' h' + (2 * sz) + ' l-' + sz + ',' + (sz * 1.6) + ' z';
        var left = x > W - 30, tx = left ? x - 12 : x + 12, anchor = left ? 'end' : 'start';
        var letter = bk.cross ? '!' : (bk.name === 'клингер' ? 'К' : 'В');
        g += '<g class="turn"><circle cx="' + x.toFixed(1) + '" cy="' + ya + '" r="8" fill="none" stroke="' + col + '" stroke-width=".7" opacity=".7"/></g>'
          + (bk.cross ? '' : '<path d="' + tri + '" fill="' + col + '" opacity="' + op + '" data-tip="' + esc(bkTip(bk)) + '"/>')
          + '<text class="bk" x="' + tx.toFixed(1) + '" y="' + (ya + 3.2) + '" text-anchor="' + anchor + '" fill="' + col + '" opacity="' + op + '" style="--c:' + col + '" data-tip="' + esc(bkTip(bk)) + '">' + letter + '</text>';
      });
      ...
      var frays = [[52, -9, 6.4, 0], [96, -5, 7.2, .9], [140, -2, 5.8, 1.7], [184, 2, 6.9, .5], [228, 5, 7.5, 1.3], [272, 9, 6.1, 2.1]]
    ...
    (function () {
      ...
      (function () {
        ...
        // РЕЙКА (14.09 вечер, владелец): букв в стрелках нет, названия над кольцами остаются,
        // ПРОЗРАЧНОСТЬ стрелки — сила движения. Одна стрелка на слот (раньше клалась дважды: сплошная
        // и поверх неё с прозрачностью — нижняя перекрывала всё, сила не читалась).
        var byKind = {}; _EV.forEach(function (e) { if (e.t >= t0 && e.t <= tE) byKind[e.kind] = e; });
        // бар часовой ленты, в который попало событие — из него берём числа для силы
        function barAt(t) { var b = null; for (var q = 0; q < _hrs.length; q++) { if (_hrs[q][0] <= t) b = _hrs[q]; else break; } return b; }
        function clamp(v) { return Math.max(0, Math.min(1, v)); }
        // СИЛА 0..1 — у каждого вида своя мерка, общей линейки нет:
        //   интерес вверх — прирост интереса на баре: +1.5% едва видна, +7.5% полная;
        //   интерес вниз  — отношение интереса к цене: ×1.5 едва видна, ×3 полная (правило 11.09);
        //   сила          — ход цены на баре к медиане |хода| по окну: 1× едва, 3× полная;
        //   пузырь        — ясный полная, спорный вполсилы;
        //   клингер       — крест полная, схождение вполсилы, иначе по разрыву линий;
        //   стык          — подхватил: оборот к норме сессии 1× вполсилы → 3× полная; не подхватил — едва;
        //   вортекс       — разрыв линий: 0.45 (порог входа) едва → 0.9 полная; выход без разрыва — 0.8;
        //   конец         — уход интереса на баре: −2% едва, −6% полная.
        var _absMed = (function () { var a = _hrs.map(function (b) { return Math.abs(+b[3] || 0); }).filter(function (v) { return v > 0; }).sort(function (x, y) { return x - y; }); return a.length ? a[Math.floor(a.length / 2)] : 1; })();
        function railStr(e) {
          var b = barAt(e.t) || [], oi = +b[2] || 0, px = +b[3] || 0;
          switch (e.kind) {
            case 'oi':    return e.dir === 'up' ? clamp((oi - 1.5) / 6) : clamp(((e.ratio || 0) - 1.5) / 1.5);
            case 'force': return clamp((Math.abs(px) / Math.max(1e-9, _absMed) - 1) / 2);
            case 'bub':   return e.sure === 'ясный' ? 1 : .5;
            case 'kl':    return has(e.str) ? e.str : .6;
            case 'sess':  return e.answered ? clamp(.5 + ((e.volx || 1) - 1) / 4) : .25;
            case 'vx':    return has(e.gap) ? clamp((e.gap - 0.45) / 0.45) : .8;
            case 'end':   return clamp((-oi - 2) / 4);
          }
          return .6;
        }
        // МЕДЛЕННЫЕ КАК СОБЫТИЯ ДЛЯ РЕЙКИ (14.09 вечер, владелец: «добавь надписи 30м 4ч»): клингер 4ч и
        // вортекс 4ч — состоянием: стрелка по повороту/кресту, время — креста, если оно известно.
        (function () {
          var K = s.klinger; if (K && (K.crossUp || K.crossDn || K.above !== undefined)) {
            var kd = K.crossUp ? 'up' : K.crossDn ? 'down' : K.above ? 'up' : 'down', ago = kd === 'up' ? K.upAgo : K.dnAgo;
            byKind.kl4 = { t: has(ago) ? tE - ago * 4 * 36e5 : tE, dir: kd, col: kd === 'up' ? '#4fd1a8' : '#ff7a7a', str: (K.crossUp || K.crossDn) ? 1 : .45, noTime: !has(ago),
              tip: 'клингер 4ч · ' + (K.crossUp ? 'крест вверх' : K.crossDn ? 'крест вниз' : K.above ? 'выше сигнала' : 'ниже сигнала') + ' · KVO ' + f(K.kvo) + ' при сигнале ' + f(K.sig) }; }
          var V = s.vx4; if (V && V.turn) {
            var n = Math.max(+((V.minus_rise || {}).n) || 0, +((V.plus_fall || {}).n) || 0, +((V.plus_rise || {}).n) || 0, +((V.minus_fall || {}).n) || 0);
            byKind.vx4 = { t: tE, dir: V.turn, col: V.turn === 'down' ? '#ff7a7a' : '#4fd1a8', str: clamp(n / 4), noTime: true,
              tip: 'вортекс 4ч · поворот ' + (V.turn === 'down' ? 'вниз' : 'вверх') + ' · ' + n + ' бар · покупатели ' + f(V.vi_plus) + ', продавцы ' + f(V.vi_minus) }; }
        })();
        window.__RAILHTML = (function () {
          // один ряд: события и медленные индикаторы. Быстрые вортекс и клингер (30м) здесь больше не стоят —
          ...
          ORD.forEach(function (k, n) {
            ...
            var tm = e ? (e.noTime ? '' : (function (t) { var d = new Date(t); return pad(d.getHours()) + ':' + pad(d.getMinutes()); })(e.t)) : '';
            h += '<i class="' + (e ? 'on ' + (up ? 'up' : 'dn') : 'off') + '" style="--st:' + st.toFixed(2) + ';animation-delay:' + (n * 70) + 'ms" title="' + esc(e ? (e.tip + ' · сила ' + Math.round(st * 100) + '%') : (k[1] + ' ' + k[2] + ' \u2014 за окно событий нет')) + '">'
              + '<span class="nm"><svg viewBox="0 0 26 26">' + (e ? '<path d="' + (up ? 'M4,20 h18 l-9,-15 z' : 'M4,6 h18 l-9,15 z') + '" fill="' + col + '" opacity="' + op + '"/>' : '<path d="M7,13 h12" stroke="' + col + '" stroke-width="2" opacity=".6"/>') + '</svg>'
              + '<u>' + esc(k[1]) + '</u></span>'
              + '<em>' + esc(k[2]) + (tm ? ' · <b>' + tm + '</b>' : (e ? '' : ' · \u2014')) + '</em></i>';
          });
          return h + '</div>';
        })();
      })();
    ...
    (function () {
      ...
      function tick() {
        ...
        var now = Date.now(), next = null, last = null;
        FS.sess.forEach(function (se) { var d0 = Math.floor(now / dayMs) * dayMs, t = d0 + se[0] * 36e5;
        ...
        // перед стыком — подхват той сессии, что идёт сейчас (последнее открытие), а не последний посчитанный стык:
        // 16.09 перед Токио висел «Нью-Йорк подхватил», хотя уже шёл Сидней
        if (post) lines.push(j && j.ok !== null ? '<div class="ml ' + (j.ok ? 'ok' : 'no') + '">' + (j.ok ? 'подхватил' : 'не подхватил') + ' · ' + j.short.replace('оборот ', '') + '</div>'
                              : '<div class="ml wait">проба идёт · ждём два закрытых бара</div>');
        else lines.push(j && j.ok !== null ? '<div class="ml ' + (j.ok ? 'ok' : 'no') + '">' + esc(last[1][1]) + ' ' + (j.ok ? 'подхватил' : 'не подхватил') + ' · ' + j.short.replace('оборот ', '') + '</div>'
                                           : '<div class="ml wait">' + esc(last[1][1]) + ' · подхват не прочитан</div>');
    ...
    (function () {
      ...
      // ПОДСКАЗКИ ПЛИТЫ БЫСТРЫХ (15.09, владелец: «не выводится подсказка»): родной <title> внутри плиты с
      // 3D-поворотом не всплывает; вместо него строка под осью — что под курсором, то и написано.
      var tipStart = function () {
        var pl = root.querySelector('.mini.fast'), tip = pl && pl.querySelector('.ftip'), sv = pl && pl.querySelector('svg'); if (!tip || !sv) return false;
        sv.addEventListener('mousemove', function (ev) { var e = ev.target && ev.target.closest ? ev.target.closest('[data-tip]') : null; tip.textContent = e ? e.getAttribute('data-tip') : ''; tip.style.opacity = e ? 1 : 0; });
        sv.addEventListener('mouseleave', function () { tip.style.opacity = 0; });
        return true;
      };
      setTimeout(function () { if (!tipStart()) setTimeout(tipStart, 400); }, 200);
    })();
    fit();
  }
