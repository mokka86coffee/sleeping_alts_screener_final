...
from core_config import ANOMALY_PATH, LEADERS_PATH
...
def _head(snapshot: RunSnapshot) -> str:
    ...
    #   4) BTC.D — core_binance.get_btc_dominance() (Ч-9 тех.долга
    #      закрыт: раньше здесь была константа-заглушка).
    # =====================================================================
    reg = getattr(snapshot, "market_regime", None) or {}
    label = str(reg.get("label", "risk-off")).upper()
    _dom = get_btc_dominance()
    dom_txt = f"{_dom:.0f}%" if _dom is not None else "—"
    try:
