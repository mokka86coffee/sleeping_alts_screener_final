...
(function () {
  ...
  /* Временный мост к сводке, пока она живёт в этом же документе.
     Порогов здесь больше нет вовсе: phase и toStop — тонкие читалки
     полей, посчитанных в analytics_stars, а stars и market приходят
     готовыми из питона. То есть отдаётся не «внутренности орбиты», а
     те же данные, которые сводка получит сама, когда переедет в свой
     iframe и соберёт их вызовом build_stars() при сборке своего файла.

     УДАЛИТЬ вместе с переездом сводки: в отдельных документах общего
     window нет, и эта строка станет мёртвой. */
