...
(function () {
  ...
  /* ВЕС ЗВЕЗДЫ (03.09, владелец): чем ярче и больше — тем ближе к
     центру. Размер несёт свежесть (s.f), яркость — темп (rate), лидер
     получает надбавку. Ровно те же величины, что рисуют звезду ниже,
     так что расположение и вид не расходятся. */
  function starWeight(s) {
    var rt = rate(s);
    var rn = rt === null ? 0.5 : Math.max(0, Math.min(1, rt / 20));
    return Math.min(1, 0.5 * (s.f || 0) + 0.5 * rn + (s.lead ? 0.15 : 0));
  }

  function starSpot(sym, idx, w) {
    var h = hash(sym);
    /* Целевой радиус от веса. Сильные — ВНУТРИ кольца, у карточки
       (0.60…0.82 от радиуса орбиты); слабые — снаружи (1.18…1.36).
       Между ними полоса самого кольца, туда не садимся. Угол — из хеша,
       чтобы соседи по весу не легли в ряд. */
    w = (w == null) ? 0.5 : w;
    /* Расстояние меряем от центра ПОЛЯ, не кольца: поле 810×380, кольцо
       372×200 — по вертикали снаружи кольца места нет, и цель «за
       кольцом» упиралась в край и съезжала на обод. Цель в долях
       полуширины/полувысоты поля: сильная звезда 0.42, слабая 0.98.
       Обод кольца по-прежнему обходим проверкой band ниже. */
    var HW = (SKY.x1 - SKY.x0) / 2, HH = (SKY.y1 - SKY.y0) / 2;
    var target = 0.42 + (1 - w) * 0.56;
    for (var k = 0; k < 60; k++) {
      var ang = ((h >>> (k % 8)) % 1000) / 1000 * Math.PI * 2;
      var jit = ((h >>> ((k + 5) % 12)) % 1000) / 1000 - 0.5;
      var d0 = Math.max(0.36, Math.min(1.0, target + jit * (0.08 + k * 0.014)));
      var x = CX + Math.cos(ang) * HW * d0;
      var y = CY + Math.sin(ang) * HH * d0;
      x = Math.min(SKY.x1, Math.max(SKY.x0, x));
      y = Math.min(SKY.y1, Math.max(SKY.y0, y));
      ...
      var tooClose = false;
      /* Зазор тает с попытками: 96 → 48. Восемьдесят звёзд с зазором
         96 в это поле не входят — карточка и обод съедают половину, и
         лишнее падало в запасную спираль друг на друга. */
      var minGap = Math.max(48, 96 - k * 1.0);
      for (var m = 0; m < PLACED.length; m++) {
        if (Math.hypot(PLACED[m].x - x, PLACED[m].y - y) < minGap) { tooClose = true; break; }
      }
      /* обод кольца обходим только в первых попытках; когда тесно —
         разрешаем: звезда на ободе лучше двух звёзд одна на другой */
      var bandOK = band > 0.18 || k >= 24;
      ...
      if (bandOK && !inCard && !nearNode && !tooClose) {
        PLACED.push({ x: x, y: y });
        return { x: x, y: y };
      }
      h = (h * 16777619 + 1) >>> 0;
    }
    /* Запасная раскладка (03.09): не спираль по золотому углу с
       фиксированного радиуса, а расширение ОТ ЦЕЛЕВОГО расстояния
       наружу с малым зазором — чтобы и в тесноте слабая звезда
       оставалась дальше сильной. */
    for (var t2 = 0; t2 < 160; t2++) {
      var ga2 = (idx * 2.39996 + t2 * 0.61803 * Math.PI * 2) % (Math.PI * 2);
      var d2 = Math.min(1.0, target + t2 * 0.004);
      var cx2 = Math.min(SKY.x1, Math.max(SKY.x0, CX + Math.cos(ga2) * HW * d2));
      var cy2 = Math.min(SKY.y1, Math.max(SKY.y0, CY + Math.sin(ga2) * HH * d2));
      if (Math.abs(cx2 - CX) < 185 && Math.abs(cy2 - CY) < 140) continue;
      var busy2 = false;
      for (var u2 = 0; u2 < PLACED.length; u2++) {
        if (Math.hypot(PLACED[u2].x - cx2, PLACED[u2].y - cy2) < 44) { busy2 = true; break; }
      }
      if (!busy2) { PLACED.push({ x: cx2, y: cy2 }); return { x: cx2, y: cy2 }; }
    }
  ...
  function buildStars() {
    ...
    /* Места раздаются по убыванию веса: сильная звезда берёт место у
       центра первой, слабая довольствуется тем, что осталось снаружи. */
    var SPOT = {};
    STARS.map(function (s, i) { return i; })
         .sort(function (a, b) { return starWeight(STARS[b]) - starWeight(STARS[a]); })
         .forEach(function (i) { SPOT[i] = starSpot(STARS[i].t, i, starWeight(STARS[i])); });
