...
(function () {
  ...
  /* Ярус монеты определяет ДЕЙСТВИЕ (analytics_action, поле act):
     оно уже посчитано в данных и знает про транш, поток, распад
  ...
  /* ═══ ИНВАРИАНТ ЗАЛА ═══
     Каждая монета журнала стоит РОВНО В ОДНОЙ группе, и сумма трёх
     групп всегда равна журналу. Группы — слой поверх журнала, а не
     сито: журнал есть запись того, что мы уже отобрали, и терять из
     него монеты нельзя ни при какой раскладке.
     Неизвестная или отсутствующая группа — это «брать»: монета без
     позиции и без срока именно там и должна быть. Разбор действия
     когда-нибудь изменится, и молча ронять монету в дыру между
     группами он не должен. */
  ...
  (function () {
    STARS.forEach(function (s) {
      ...
      var g = (s.act && s.act.group) || 'take';
      if (!GROUPS_ALL[g] || g === 'hold') g = 'take';
      GROUPS_ALL[g].push(s);
    });
    /* Инвариант: ТОРГОВЫЕ группы в сумме дают журнал. HOLD считается
       отдельно и в сумму не входит — иначе каждая монета посчиталась
       бы дважды. */
    var sum = GROUPS_ALL.take.length + GROUPS_ALL.trade.length +
              GROUPS_ALL.exit.length;
    if (sum !== STARS.length && window.console) {
      console.warn('зал: в торговых группах ' + sum + ' из ' + STARS.length);
    }
  })();
