...
(function () {
  ...
  var HELD_ANY = STARS.some(function (s) {
    var g = (s.act && s.act.group) || 'take';
    return g === 'hold' || g === 'exit';
  });
  var GROUP = HELD_ANY ? 'exit' : 'take';
  ...
  var GROUPS_ALL = { exit: [], hold: [], take: [] };
  (function () {
    STARS.forEach(function (s) {
      var g = (s.act && s.act.group) || 'take';
      if (!GROUPS_ALL[g]) g = 'take';
      GROUPS_ALL[g].push(s);
    });
    var sum = GROUPS_ALL.exit.length + GROUPS_ALL.hold.length +
              GROUPS_ALL.take.length;
    if (sum !== STARS.length && window.console) {
      console.warn('зал: монет в группах ' + sum + ' из ' + STARS.length);
    }
  })();
    ...
    plan.forEach(function (p) {
      ...
      list.forEach(function (c, i) {
        ...
        var y = rowY[p.rows === 1 ? 0
          : (centered ? (Math.abs(slot) % p.rows) : (i % p.rows))];
        var sc = stratOf(c);
