...
SCREENS = ("intro", "dashboard", "brief", "podium", "coin")

# Экран, с которого начинается отчёт.
START_SCREEN = "intro"          # 06.09: первый экран — «поведение света» (брать / держать / закрыть)
...
SEQUENCE = {
    "intro": "brief",       # 06.09: интро → сводка
...
(function () {{
  ...
  window.addEventListener('message', function (e) {{
    ...
    // Открыть экран с якорем (06.09): интро → монета по клику на имя; сам экран монеты
    // грузится в тот же iframe, якорь он читает из location.hash — оболочке
    // достаточно сменить экран, iframe уже ушёл по ссылке
    if (msg.type === 'ob:open') {{
      current = String(msg.screen || '');
      return;
    }}
