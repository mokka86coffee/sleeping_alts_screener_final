...
#    ПОСЛЕ ×3 от дна) ─────────────────────────────────────────────
# Опоздание было не в шаблоне, а в ОКНЕ: лестница считалась неделей и
...
LADDER_EARLY_DAYS = 4    # окно раннего распознавания лестницы
...
# разойдутся молча.
PLOT_MOVING_MARKS = (
    "НА ХОДУ", "ИСКРА", "подтверждение пришло", "рука ушла",
    "финал лестницы", "раздача после пика", "лестница на ходу",
)
...
def plot_line(tr: list, oh: list, fu: list, oi: list) -> str:
    ...
    if vol_now >= TRIGGER_VOL_X and fu_last < TRIGGER_FUND_MAX:
        return ("взведён курок по связке (оборот и фандинг): оборот ×"
    ...
    if px_wk > LADDER_LATE_MIN and wk_delta < 0 and fu_last < 0.05:
        return ("лестница на ходу (шаблон PROM/STX): за неделю цена +"
                ...
                "совпадут с падением 7%+, это финал лестницы")
    E = LADDER_EARLY_DAYS
    if len(px) > E and px[-(E + 1)]:
        ...
        if (LADDER_EARLY_MIN <= px_e <= LADDER_LATE_MIN
                ...
                and fu_last < 0.05):
            return ("лестница руки, ранняя стадия (шаблон PROM/STX): "
...
def plot_stage(plot: str) -> str:
    ...
    (курок, набор кита, ранняя лестница), а 'moving' показывать
    ...
    # и в нём законно стоят слова другой стадии: «рука над сливом …
    # первый день, когда продажи продавили цену, — рука ушла». Поиск
...
def live_refresh(entry: dict, live: dict) -> dict:
    ...
    elif "лестниц" in pl and d < 0 and px < -7:
        # Корень «лестниц» ловит ОБЕ стадии (правка 01.09): после
        # переименования поздней в «лестница на ходу» проверка по
        # «лестница руки» тихо перестала бы её видеть, а финал у них
