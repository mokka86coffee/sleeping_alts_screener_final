...
def plot_line(tr: list, oh: list, fu: list, oi: list, lq: list | None = None,
    ...
    # 0б. РАЗГОН НА СПРОСЕ (05.09, случай USELESS): не сквиз, а покупки —
    # за три дня цена +30%+, тейкер > 1 каждый из трёх дней, дельта в плюс
    # и растёт, плечо удвоилось (лонги открывают), а шортов сгорело мало
    # против дельты. Цели нет (сверху чистое поле) — есть условие выхода.
    # Переполнение: фандинг ≥ 0.05 или плечо росло вдвое быстрее цены —
    # снять половину, толпа переплачивает.
    if len(px) >= 4 and px[-4]:
        px_3 = px[-1] / px[-4] - 1
        tks = [(t["quote_buy_volume"] / max(1.0, t["quote_sell_volume"])) for t in tr[-3:]]
        oi_v = [r.get("open_interest") or 0 for r in (oi or [])]
        oi_3 = (oi_v[-1] / oi_v[-4] - 1) if len(oi_v) >= 4 and oi_v[-4] else 0.0
        liq_s = sum((r.get("short_liquidations_usd") or 0) for r in (lq or [])[-3:]) if lq else 0.0
        d3 = sum(dl[-3:])
        if (px_3 >= 0.30 and all(t > 1.0 for t in tks) and d_now > 0 and d3 > 0
                and oi_3 >= 0.5 and (liq_s < 0.3 * d3 or not lq)):
            v3 = sum(vols[-3:]) / (3 * med)
            base = (f"разгон на спросе (шаблон USELESS): за 3 дня цена {px_3 * 100:+.0f}% при покупках каждый день, "
                    f"дельта {d3 / 1e6:+.1f}M, шортов сгорело {liq_s / 1e6:.1f}M — это не сквиз, а деньги; "
                    f"плечо {oi_3 * 100:+.0f}%, оборот ×{v3:.0f} к норме")
            crowded = fu_last >= 0.05 or (oi_3 > 2 * px_3 and px_3 > 0)
            if crowded:
                return base + (f"; ПЕРЕПОЛНЕНО: фандинг {fu_last:.3f}%, плечо растёт быстрее цены — снять половину, "
                               "остаток по низу суток; параболы отдают половину за день")
            return base + ("; цели нет — сверху чистое поле, есть условие выхода: первый день с дельтой в минус "
                           "или закрытие ниже семидневной средней; стоп — по низу суток")

    # 0в. РАЗГОН ОТПУСТИЛ (05.09) — выход из предыдущего шаблона: был разгон на
    # покупках (за 5 дней +30%+ при тейкере > 1 в трёх из четырёх предыдущих),
    # а сегодня либо дельта в минус на обороте не ниже половины среднего за
    # три дня, либо закрытие ниже семидневной средней. Первый такой день —
    # выход без иллюзий: параболы отдают половину за день.
    if len(px) >= 7 and px[-6] and all(px[-7:]):
        px_5 = max(px[-5:]) / px[-6] - 1
        tks_prev = [(t["quote_buy_volume"] / max(1.0, t["quote_sell_volume"])) for t in tr[-5:-1]]
        ma7 = sum(px[-7:]) / 7
        v3m = sum(vols[-4:-1]) / 3 if len(vols) >= 4 else med
        sell_day = d_now < 0 and vols[-1] >= 0.5 * v3m
        below_ma = px[-1] < ma7
        if px_5 >= 0.30 and sum(1 for t in tks_prev if t > 1.0) >= 3 and (sell_day or below_ma):
            why = []
            if sell_day:
                why.append(f"дельта {d_now / 1e6:+.1f}M на обороте ×{vols[-1] / med:.0f} к норме")
            if below_ma:
                why.append(f"закрытие {px[-1]:.4g} ниже семидневной средней {ma7:.4g}")
            return ("разгон отпустил (выход из шаблона USELESS): после +" + f"{px_5 * 100:.0f}% за 5 дней на покупках "
                    "сегодня " + " и ".join(why) + " — первый день продаж после параболы; выходить, "
                    "возврат интереса только через новый набор, ближайшая полоса лонгов под ценой — цель отката")
